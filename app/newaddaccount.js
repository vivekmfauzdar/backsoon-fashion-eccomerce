...
function Newaddaccount({curUserId, cancelBtn}) {
    ...
    useEffect(() => {
      ...
      dbfs.collection("Orders").doc(curUserId).collection("addresses").doc(id).set(data).then(() => {
    ...
    return (
        ...
        <button
          ...
          <button className='text-white p-2 mt-3 bg-blue-500' onClick={() => cnclBtn()}>Cancel</button>
        </div>
